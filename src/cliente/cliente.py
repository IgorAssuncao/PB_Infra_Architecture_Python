import sys
import socket
import pickle

def tracinhos(cor):
    if cor.lower() == 'azul':
        print('\033[1;34m-\033[0;0m' * 50)
    if cor.lower() == 'verde':
        print('\033[1;32m-\033[0;0m' * 50)
    if cor.lower() == 'vermelho':
        print('\033[1;31m-\033[0;0m' * 50)

def pipe_colorido(cor):
    if cor.lower() == 'azul':
        print('\033[1;34m|\033[0;0m')
    if cor.lower() == 'verde':
        print('\033[1;32m|\033[0;0m')
    if cor.lower() == 'vermelho':
        print('\033[1;31m|\033[0;0m')

def mount_menu():
    print()
    tracinhos('azul')
    print("""\033[1;36mProjeto de Bloco\033[0;32m\nOpcoes:\033[0;0m
          0 - Sair
          1 - Informacoes da CPU
          2 - Informacoes da Memoria
          3 - Informacoes do Disco
          4 - Informacoes da Rede
          5 - Informacoes de Diretorios""")
    tracinhos('azul')
    print()

def format_resposta(opcao, resposta):
    if opcao == '0':
        tracinhos('verde')
        print("""|Conexao encerrada|""")
        tracinhos('verde')
    if opcao == '1':
        tracinhos('verde')
        print(f"""CPU:
          {pipe_colorido('verde')} Nome: {resposta[0]} {pipe_colorido('verde')}
          {pipe_colorido('verde')} Arquitetura: {resposta[1]} bits {pipe_colorido('verde')}
          {pipe_colorido('verde')} Palavra: {resposta[2]} {pipe_colorido('verde')}
          {pipe_colorido('verde')} Quantidade de nucleos: {resposta[3]} {pipe_colorido('verde')}
          {pipe_colorido('verde')} Quantidade de threads: {resposta[4]} {pipe_colorido('verde')}
          {pipe_colorido('verde')} Frequencia minima: {resposta[5]} MHz {pipe_colorido('verde')}
          {pipe_colorido('verde')} Frequencia maxima: {resposta[6]} MHz {pipe_colorido('verde')}
          {pipe_colorido('verde')} Frequencia atual: {resposta[7]} MHz {pipe_colorido('verde')}
          {pipe_colorido('verde')} Porcentagem de uso: {resposta[8]}% {pipe_colorido('verde')}""")
        for i in range(len(resposta[9])):
            print(f"""|\t  Porcentagem de uso nucleo {i + 1}: {resposta[9][i]} % |""")
        tracinhos('verde')
    if opcao == '2':
        tracinhos('verde')
        print(f"""Memoria:
           | Memoria total: {resposta[0]/1024/1024/1024:.2f} GB |
           | Memoria disponivel: {resposta[1]/1024/1024/1024:.2f} GB |
           | Memoria em uso: {resposta[2]/1024/1024/1024:.2f} GB |
           | Percentual de Uso: {resposta[3]/1024/1024/1024:.2f} GB |
           | Percentual disponivel: {resposta[4]/1024/1024/1024:.2f} GB |""")
        tracinhos('verde')
    if opcao == '3':
        print(resposta)
        tracinhos('verde')
        print(f"""Disco:
          | Armazenamento total: {resposta[0]/1024/1024/1024:.2f} GB |
          | Armazenamento em uso: {resposta[1]/1024/1024/1024:.2f} GB |
          | Percentual em uso: {resposta[2]/1024/1024/1024:.2f} % |
          | Armazenamento disponivel: {resposta[3]/1024/1024/1024:.2f} GB |""")
        tracinhos('verde')

def imprime_status(bytes, tam):
    kbytes = bytes/1024
    tam_bytes = tam/1024
    texto = """Baixando..."""
    texto += f"""{kbytes} KB"""
    texto += f"""de {tam_bytes} KB"""
    print(texto)

opcoes = [str(i) for i in range(10)]

host, port = socket.gethostname(), 9991

cliente = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

try:
    cliente.connect((host, port))
except Exception as error:
    print(error)
    sys.exit(1)

running = True
while running:
    mount_menu()
    try:
        opcao = input("""Opcao: """)
        if opcao not in opcoes:
            print('Opcao invalida, tente novamente')
            mount_menu()
            opcao = input("""Opcao: """)
        cliente.send(opcao.encode())
        resposta = pickle.loads(cliente.recv(1024))
        if resposta:
            format_resposta(opcao, resposta)
        if opcao == '0':
            running = False
    except Exception as error:
        print(error)
        running = False

cliente.close()
